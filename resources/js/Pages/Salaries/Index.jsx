...
export default function SalariesIndex({ salaries, doctors, auth }) {
    const [searchTerm, setSearchTerm] = useState('');
    const [filterStatus, setFilterStatus] = useState('all');

    // Role-based permissions
    const userRole = auth.user.role;
    const canManageSalaries = userRole === 'admin';
    const canCreateSalaries = userRole === 'admin';
    const canEditSalaries = userRole === 'admin';
    const canDeleteSalaries = userRole === 'admin';

    // If reception or doctor tries to access this page, show access denied
                ...
                <Card>
                    <CardContent className="text-center py-12">
                        ...
                        <p className="text-gray-500">
                            {userRole === 'reception' && 'Reception staff do not have access to salary management.'}
                            {userRole === 'doctor' && 'Doctors do not have access to salary management.'}
                        </p>
                    </CardContent>
                </Card>
            </AuthenticatedLayout>
                ...
                {filteredSalaries.length === 0 && (
                    <Card>
                        <CardContent className="text-center py-12">
                            ...
                            {canCreateSalaries && !searchTerm && filterStatus === 'all' && (
                                <Button>
                                    <Plus className="mr-2 h-4 w-4" />
                                    Add New Salary Record
                                </Button>
